...
namespace FrameAnalysisProgram.INPUT_OUTPUT
{
    ...
    public class ResultPrinter
    {
        ...
        public void PrintReactions(FrameAnalysisResult result)
        {
            if (result == null)
                throw new ArgumentNullException(nameof(result));
            ...
            Console.WriteLine();
            Console.WriteLine("========================================");
            Console.WriteLine("SUPPORT REACTIONS (GLOBAL COORDINATES)");
            Console.WriteLine("========================================");
            Console.WriteLine("Node\tFx\tFy\tMz");

            foreach (NodalReaction reaction in result.Reactions)
            {
                string fx = reaction.HasFx ? $"{reaction.Fx:G10}" : "-";
                string fy = reaction.HasFy ? $"{reaction.Fy:G10}" : "-";
                string mz = reaction.HasMz ? $"{reaction.Mz:G10}" : "-";

                Console.WriteLine($"{reaction.NodeId}\t{fx}\t{fy}\t{mz}");
            }
        }
        ...
        private void PrintElements(StructureModel model)
        {
            ...
            Console.WriteLine("========================================");
            Console.WriteLine("Id\tStart\tEnd\tMat\tSec\tType\tRelease\tLength");

            foreach (StructuralElement2D element in model.Elements)
            {
                string type = element is TrussElement2D ? "Truss" : "Frame";
                string release = element is FrameElement2D frame ? frame.Release.ToString() : "-";

                Console.WriteLine(
                    $"{element.Id}\t{element.StartNode.Id}\t{element.EndNode.Id}\t{element.Material.Id}\t{element.Section.Id}\t{type}\t{release}\t{element.Length:G10}");
            }
        }
