...
namespace FrameAnalysisProgram.ANALYSIS_CORE
{
    ...
    public class GlobalStiffnessAssembler
    {
        ...
        private void AssembleElement(SparseMatrix globalMatrix, StructuralElement2D element, DofMap dofMap)
        {
            double[,] elementGlobalStiffness = element.GetGlobalStiffnessMatrix();
            int[] elementDofMap = element.GetGlobalDofIndices(dofMap);

            int dofCount = elementDofMap.Length;

            for (int p = 0; p < dofCount; p++)
            {
