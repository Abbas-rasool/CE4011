    ...
    public class ElementForceRecovery
    {
            ...
            CustomVector globalDisplacementVector)
        {
            ...
            foreach (StructuralElement2D element in model.Elements)
            {
                ...
                if (element is FrameElement2D frame)
                {
                    // Total fully-fixed fixed-end forces from every span load on this member.
                    double[] fixedEndForces = SumMemberLoadFixedEndForces(model, frame);

                    // {Q} = [k]{d} + {Ff_released}
                    localEndForces = frame.GetLocalEndForceVector(globalElementDisplacements, fixedEndForces);
                }
                else
                {
                    // Truss and other elements carry no span loads: {Q} = [k]{d}.
                    localEndForces = element.GetLocalEndForceVector(globalElementDisplacements);
                }
